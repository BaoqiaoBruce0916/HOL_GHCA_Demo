...
The home page includes accessible filters for category and publisher, and the filters can be combined to narrow the game list without leaving the static site. Selections are kept in the URL query string, so the catalog can be filtered with a bookmarkable, shareable link.
